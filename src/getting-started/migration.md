...
**Note**: Adding the `type="module"` attribute also affects the loading behavior of scripts. Classic scripts are "render blocking", meaning the rest of the HTML document is not parsed until script execution is complete. Module scripts are not render blocking, and execution is deferred until the HTML is fully parsed (keep in mind, that Parcel inserts `defer` attribute automatially for older browsers and in development mode). This means features like `document.write` do not work in module scripts. See [official docs](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Modules#other_differences_between_modules_and_standard_scripts) to learn more about differences between modules and classic scripts. If you are relying on these features, either migrate to modern APIs or continue using a classic script for that part of your app.
{% enderror %}
