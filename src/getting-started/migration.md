...
Many `package.json`s (e.g. the one generated by `npm init`) contains `main: "index.js"` which ignored by most tools (for non-library projects). Parcel 2 will however use that value as the output path; for most web apps, this line should simply be removed. If really you do need it, you can add `"targets": { "main": false }` to your `package.json`. See [Configuration#main](/configuration/package-json/#main-%2F-module-%2F-browser)
